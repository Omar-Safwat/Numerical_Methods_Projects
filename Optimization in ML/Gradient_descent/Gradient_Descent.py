...
class Momentum_GD(Gradient_Descent):
    ...
    def initialize(self, alpha, guess, gamma, batch_size):
        ...
        self.gamma = gamma
        self.nu = self.gamma * self.J_prime(self.h_pred, self.y_train[:batch_size], self.X_train[:batch_size, :], batch_size)
    ...
    def update_weights_GD(self, idx_1, idx_2, theta):
        ...
        grad_new = self.J_prime(self.h_pred, self.y_train[idx_1 : idx_2], self.X_train[idx_1 : idx_2, :], self.n_points)
        self.grad_hist = np.hstack((self.grad_hist, grad_new))
        self.nu = self.gamma * self.nu + self.alpha * grad_new
        theta = theta - self.nu
        # Record in history
        self.theta_hist = np.hstack((self.theta_hist, theta))
        self.h_pred = self.H_func(self.X_train[idx_1 : idx_2, :], theta)
...
class Nesterov_GD(Momentum_GD):
    ...
    def update_weights_GD(self, idx_1, idx_2, theta):
        ...
        mu_proj = self.gamma * self.nu + self.alpha * grad_proj
...
class Adagrad(Gradient_Descent):
    """Adagrad implementation of gradient descent"""

    def initialize(self, alpha, guess, batch_size, eps):
        super().initialize(alpha, guess, batch_size)
        self.accumulated_grad = self.grad_hist ** 2
        # Correction term added to accumulated gradient to avoid dividing by zero
        self.eps = eps 
    ...
    def update_weights_GD(self, idx_1, idx_2, theta):
        ...
        grad_new = self.J_prime(self.h_pred, self.y_train[idx_1 : idx_2], self.X_train[idx_1 : idx_2, :], self.n_points)
        self.grad_hist = np.hstack((self.grad_hist, grad_new))
        self.accumulated_grad = self.accumulated_grad + grad_new ** 2
 
        # New adaptive learning rate
        learn_rate = self.alpha / (self.accumulated_grad ** 0.5 + self.eps)

        # Update weights
        theta = theta - learn_rate * grad_new
        self.theta_hist = np.hstack((self.theta_hist, theta))
        self.h_pred = self.H_func(self.X_train[idx_1 : idx_2, :], theta)
        self.cost_hist = np.append(self.cost_hist, self.cost_func(self.h_pred, self.y_train[idx_1 : idx_2]))

    def adagrad_GD(self, guess=0, alpha=0.001, max_epochs=1e3, stop_criteria=1e-3, eps=1e-8):
        """Optimize weights using Adagrad"""

        # Initialize first epoch
        idx_2 = len(self.y_train)
        self.initialize(alpha, guess, idx_2, eps)
        self.MAX_EPOCHS = max_epochs
        self.stop_criteria = stop_criteria
...
class RMSprop(Gradient_Descent):
    """RMSprop implementation of gradient descent"""

    def initialize(self, alpha, guess, batch_size, eps, beta_grad):
        super().initialize(alpha, guess, batch_size)
        # Correction term added to accumulated gradient to avoid dividing by zero
        self.eps = eps 
        # Constant to exponentially weight the accumulated gradient history.
        self.beta_grad = beta_grad 
        bias_correction = 1 - self.beta_grad 
        self.accumulated_grad = (1 - self.beta_grad) * self.grad_hist ** 2 / bias_correction
    ...
    def update_weights_GD(self, idx_1, idx_2, theta):
        ...
        grad_new = self.J_prime(self.h_pred, self.y_train[idx_1 : idx_2], self.X_train[idx_1 : idx_2, :], self.n_points)
        self.grad_hist = np.hstack((self.grad_hist, grad_new))
        self.accumulated_grad = self.beta_grad * self.accumulated_grad + (1 - self.beta_grad) * grad_new ** 2

        # Apply bias correction
        bias_correction = 1 - self.beta_grad ** (self.epoch + 1)
        self.accumulated_grad = self.accumulated_grad / bias_correction
    ...
    def rms_prop_GD(self, guess=0, alpha=0.001, max_epochs=1e3, stop_criteria=1e-3, eps=1e-8, beta_grad=0.9):
        """Optimize weights using RMSprop"""

        # Initialize first epoch
        idx_2 = len(self.y_train)
        self.initialize(alpha, guess, idx_2, eps, beta_grad)
        self.MAX_EPOCHS = max_epochs
        self.stop_criteria = stop_criteria
...
class Adam(RMSprop):
    ...
    def initialize(self, alpha, guess, batch_size, eps, beta_grad, beta_nu):
        super().initialize(alpha, guess, batch_size, eps, beta_grad)
        # Constant exponentially weights the accumulated moemntum
        self.beta_nu = beta_nu
        bias_correction_2 = 1 / (1 - self.beta_nu)
        # Initialize momentum term
        self.nu = (1 - self.beta_nu) * self.grad_hist / bias_correction_2  
    ...
    def update_weights_GD(self, idx_1, idx_2, theta):
        ...
        self.grad_hist = np.hstack((self.grad_hist, grad_new))
        self.accumulated_grad = self.beta_grad * self.accumulated_grad + (1 - self.beta_grad) * grad_new ** 2
        self.nu = self.beta_nu * self.nu + (1 - self.beta_nu) * grad_new

        # Apply bias correction
        bias_correction = 1 - self.beta_grad ** (self.epoch + 1)
        self.accumulated_grad = self.accumulated_grad / bias_correction

        bias_correction_2 = 1 - self.beta_nu ** (self.epoch + 1)
        self.nu = self.nu / bias_correction_2
        
        # New adaptive learning rate
        learn_rate = self.alpha / (self.accumulated_grad ** 0.5 + self.eps)

        # Update weights
        theta = theta - learn_rate * self.nu
        self.theta_hist = np.hstack((self.theta_hist, theta))
        self.h_pred = self.H_func(self.X_train[idx_1 : idx_2, :], theta)
        self.cost_hist = np.append(self.cost_hist, self.cost_func(self.h_pred, self.y_train[idx_1 : idx_2]))

    def adam_GD(self, guess=0, alpha=0.001, max_epochs=1e3, stop_criteria=1e-3, eps=1e-8, beta_grad=0.9, beta_nu=0.9):
        """Optimize weights using Adam"""

        # Initialize first epoch
        idx_2 = len(self.y_train)
        self.initialize(alpha, guess, idx_2, eps, beta_grad, beta_nu)
        self.MAX_EPOCHS = max_epochs
        self.stop_criteria = stop_criteria
